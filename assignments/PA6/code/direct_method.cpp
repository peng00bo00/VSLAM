...
int main(int argc, char **argv) {
    ...
    for (int i = 1; i < 6; i++) {  // 1~10
        ...
        cout << "Image: " << (fmt_others % i).str() << endl;
...
void DirectPoseEstimationSingleLayer(
    ...
    for (int iter = 0; iter < iterations; iter++) {
        ...
        for (size_t i = 0; i < px_ref.size(); i++) {
            ...
            Eigen::Vector3d P = depth_ref[i] * Eigen::Vector3d((px_ref[i][0] - cx) / fx, (px_ref[i][1] - cy) / fy, 1.0);
            Eigen::Vector3d P2= T21 * P;
            ...
            for (int x = -half_patch_size; x < half_patch_size; x++)
                for (int y = -half_patch_size; y < half_patch_size; y++) {
                    ...
                    J_img_pixel(0) = 0.5 * (GetPixelValue(img2, u+1+x, v+y) - GetPixelValue(img2, u-1+x, v+y));
                    J_img_pixel(1) = 0.5 * (GetPixelValue(img2, u+x, v+1+y) - GetPixelValue(img2, u+x, v-1+y));
