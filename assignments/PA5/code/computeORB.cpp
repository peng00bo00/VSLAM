...
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints) {
	...
	// START YOUR CODE HERE (~7 lines)
        ...
        // skip the points near boundaries
        if (kp.pt.x < half_patch_size || kp.pt.y < half_patch_size ||
            kp.pt.x + half_patch_size >= image.cols || kp.pt.y + half_patch_size >= image.rows)
        {
            continue;
        }
        ...
        for (int i = -half_patch_size; i < half_patch_size; i++) {
            for (int j = -half_patch_size; j < half_patch_size; j++) {
                uchar pixel = image.at<uchar>(kp.pt.y + j, kp.pt.x + i);
                m10 += i * pixel;
                m01 += j * pixel;
            }
        }
        ...
        // get the angle
        kp.angle = std::atan(m01 / m10) / pi * 180;
...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints, vector<DescType> &desc) {
    for (auto &kp: keypoints) {
        ...
        for (int i = 0; i < 256; i++) {
            ...
            cv::Point2f pp = cv::Point2f(cos * p.x - sin * p.y, sin * p.x + cos * p.y) + kp.pt;
            cv::Point2f qq = cv::Point2f(cos * q.x - sin * q.y, sin * q.x + cos * q.y) + kp.pt;
...
void bfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches) {
    ...
    for (size_t i = 0; i < desc1.size(); i++)
    {
        ...
        for (size_t j = 0; j < desc2.size(); j++)
        {
            ...
            if (dist < d_max)
            {
                cv::DMatch match{int(i), int(j), float(dist)};
